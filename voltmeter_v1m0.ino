...
ver.1.0 - voltmeter (1.1V maxim voltage) can replace old C520 voltmeter
*/
...
#define intrare A1  // pin for read voltage and GND
float voltageref = 1100.;  // ideal value
float corectie = 0.976;  // correction factor for reading when is use multiplexing method and other causes
...
float vf = corectie * analogRead(intrare) * (voltageref / 1023.00);
  ...
  for(int digit = 4 ; digit > 0 ; digit--) {
   ...
   //   lightNumber(11); // display degree symbol
    ...
    case 4:
      ...
      digitalWrite(segDP, SEGMENT_OFF);
 //     lightNumber(22); // display U letter
      lightNumber(toDisplay % 10);
      toDisplay /= 10;
      digitalWrite(segDP, SEGMENT_OFF);
      delayMicroseconds(DISPLAY_BRIGHTNESS); 
